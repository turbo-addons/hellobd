...
class Post extends Model implements SpatieHasMedia
{
    ...
    /**
     * Get media URLs with proxy support for R2 images
     */
    public function getMediaUrlsAttribute(): array
    {
        ...
        return $this->media->map(function ($media) {
            ...
            if ($disk === 'r2') {
                // R2 storage - Use proxy
                $cdnUrl = config('filesystems.disks.r2.url') . '/' . $media->id . '/' . $media->file_name;
                $url = url('/cdn-proxy?url=' . urlencode($cdnUrl));
                $originalUrl = $cdnUrl;
            } else {
                // Local storage
    ...
    public function getFeaturedImageUrl(string $conversion = ''): ?string
    {
        ...
        // Check disk type and generate appropriate URL with proxy for R2
        $disk = $media->disk ?? 'public';
