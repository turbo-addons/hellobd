...
class PostController extends Controller
{
    ...
    public function store(StorePostRequest $request, string $postType = 'post'): RedirectResponse
    {
        ...
        // Fix media JSON columns after post creation
        $this->fixMediaJsonColumns($post->id);
    ...
    public function update(UpdatePostRequest $request, string $postType, string $id): RedirectResponse
    {
        ...
        // Fix media JSON columns after post update
        $this->fixMediaJsonColumns($post->id);
    ...
    protected function handleBuilderFeaturedImage(Request $request, Post $post): void
    {
        ...
        // Log for debugging
        \Log::info('Featured Image URL received:', ['url' => $featuredImageUrl]);
        ...
        if ($featuredImageUrl && filter_var($featuredImageUrl, FILTER_VALIDATE_URL)) {
            ...
            if ($featuredImageUrl !== $currentUrl) {
                ...
                $result = $this->mediaService->associateExistingMedia($post, $featuredImageUrl, 'featured');
                
                \Log::info('Media association result:', ['success' => $result ? 'yes' : 'no', 'media_id' => $result ? $result->id : null]);
            }
        } else {
            \Log::warning('Featured image URL invalid or missing:', ['url' => $featuredImageUrl]);
        }
    }
    ...
    /**
     * Fix media JSON columns for the given post
     * Converts string JSON to proper array format for manipulations, custom_properties, etc.
     */
    private function fixMediaJsonColumns(int $postId): void
    {
        $medias = Media::where('model_type', Post::class)
            ->where('model_id', $postId)
            ->get();

        foreach ($medias as $media) {
            // Fix manipulations column if it's a string
            if (is_string($media->manipulations)) {
                $media->manipulations = [];
            }
            
            // Fix custom_properties column if it's a string
            if (is_string($media->custom_properties)) {
                $media->custom_properties = [];
            }
            
            // Fix generated_conversions column if it's a string
            if (is_string($media->generated_conversions)) {
                $media->generated_conversions = [];
            }
            
            // Fix responsive_images column if it exists and is a string
            if (isset($media->responsive_images) && is_string($media->responsive_images)) {
